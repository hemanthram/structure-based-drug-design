...
from utils import load_file, get_action_points_plane, equation_plane
...
try: os.mkdir("./outputs_planar_1/mxQED")
except: pass
...
def generate(ix):
    ...
    os.mkdir("./outputs_planar_1/"+out_name)
    ...
    plane_eq = equation_plane(ctx_lp[0][0], ctx_lp[0][1], ctx_lp[0][2], ctx_lp[1][0], ctx_lp[1][1], ctx_lp[1][2], ctx_lp[2][0], ctx_lp[2][1], ctx_lp[2][2])
    ...
    while(not done):
        ...
        action_points = get_action_points_plane(ctx_lp, centers, limits, resolution, plane_eq, 0.1)
        ...
        done = (terminate.item() > 0.7) or (i>30); (3+i)>=lf.shape[0];  stat[len(ligand_z)] = terminate.item()
        if(done):
        ...
        try:
            ...
            with SDWriter('./outputs_planar_1/'+out_name+'/'+str(i)+'.sdf') as w:
                w.write(rdmol)
            ...
            res.save('./outputs_planar_1/'+out_name+'/_'+str(i)+'.png')
    ...
    if(mxi != 0):
        shutil.copy('./outputs_planar_1/'+out_name+'/_'+str(mxi)+'.png', './outputs_planar_1/mxQED/'+str(ix)+'.png')
        shutil.copy('./outputs_planar_1/'+out_name+'/'+str(mxi)+'.sdf', './outputs_planar_1/mxQED/'+str(ix)+'.sdf')
        shutil.rmtree('./outputs_planar_1/'+out_name)
...
with open('./outputs_planar_1/res.txt', 'w') as f:
    f.write(metrics)
...
plt.savefig('./logs/planar_model_props.png')
...
plt.savefig('./logs/planar_model_props_1.png')
...
plt.savefig('./logs/planar_model_props_2.png')
