...
def connect_the_dots(mol, atoms, indicators, maxbond=4):
    ...
    for stretch,bdist,bond in binfo:
        ...
        if stretch > 0.45 or forms_small_angle(a1,a2,cutoff=75) or forms_small_angle(a2,a1,cutoff=75):
            #don't fragment the molecule
            if not reachable(a1,a2):
                continue
            mol.DeleteBond(bond)
...
def convert_ob_mol_to_rd_mol(ob_mol,struct=None):
    ...
    try:
        # Chem.SanitizeMol(rd_mol,Chem.SANITIZE_ALL^Chem.SANITIZE_KEKULIZE,catchErrors=True)
        Chem.SanitizeMol(rd_mol,Chem.SANITIZE_KEKULIZE,catchErrors=True)
    except:
...
def reconstruct_from_generated(xyz, atomic_nums):
    ...
    mol.PerceiveBondOrders()
    # print(len(atomic_nums))
    # print(mol.NumAtoms(), 'atoms present')
    # print(mol.NumBonds(), 'bonds added')
    ...
    # pybelmol.write("sdf", "outputs/"+out_name+".sdf")

    # cmd.reinitialize()
    # cmd.load("outputs/"+out_name+".sdf")
    # cmd.png("outputs/"+out_name+".png")
...
def reconstruct_from_struct(lp, lf):
    atom_nums = []

    for i in range(len(lf)):
        atom_nums.append(ligand_z[np.argwhere(lf[i]==1)[0][0]])

    mol = reconstruct_from_generated(lp, atom_nums)
